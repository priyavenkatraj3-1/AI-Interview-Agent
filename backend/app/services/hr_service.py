"""
HR round business logic — the backend-layer glue between the persistence
models (InterviewSession, StageProgress, CostLog) and the
framework-agnostic agents/ package.

Mirrors app.services.technical_service's shape (see that module's
docstring, itself mirroring aptitude_service): this decides *when* to call
which agent for the HR stage, persists the result, and uses
agents.orchestrator.get_next_stage() for the one stage transition it's
responsible for (hr -> completed, the final stage).

All in-progress HR state (topic sequence, difficulty, question history,
the currently pending question) lives in StageProgress.details as a JSON
blob, same as every earlier stage — no new tables.
"""
...
from agents.hr_interviewer.grader import build_hr_independent_grader, build_hr_key_validator
from agents.hr_interviewer.hr_interviewer import build_hr_interviewer
...
_hr_interviewer = build_hr_interviewer()
# Two independent agents, not one: _hr_independent_grader must finish
# before _hr_key_validator is ever called (see submit_answer() below), and
# the former's call site never has access to the generator's
# model_answer/rubric_keywords — see tests/test_grader_independence.py.
_hr_independent_grader = build_hr_independent_grader()
_hr_key_validator = build_hr_key_validator()


class HRServiceError(Exception):
    """Raised for any HR-flow error; routes map status_code to an HTTPException."""
...
def _get_session(db: Session, session_id: str) -> InterviewSession:
    ...
    if session is None:
        raise HRServiceError("Session not found", status_code=404)
    return session
...
async def _generate_question(session_id: str, target_company: str, details: dict, index: int) -> tuple[dict, object]:
    ...
    try:
        result = await _hr_interviewer.run(
        ...
        raise HRServiceError(f"HR question generation failed: {exc}", status_code=502) from exc
...
async def start_hr(db: Session, session_id: str) -> dict:
    ...
    db.add(progress)
    _log_cost(db, session_id, _hr_interviewer.name, usage)
...
def get_current(db: Session, session_id: str) -> dict:
    ...
    progress = _get_stage_progress(db, session_id)
    if progress is None:
        raise HRServiceError("HR round not started for this session", status_code=409)
    return _state(session, progress)
...
async def submit_answer(db: Session, session_id: str, candidate_answer: str) -> dict:
    ...
    if progress.status == "completed":
        raise HRServiceError("HR round already completed", status_code=409)

    details = progress.details
    current_question = details.get("current_question")
    if current_question is None:
        raise HRServiceError("No pending question to answer", status_code=409)
    ...
    draft_result = await _hr_independent_grader.run(
        question=current_question["question"],
        candidate_answer=candidate_answer,
    )
    _log_cost(db, session_id, _hr_independent_grader.name, draft_result.usage)
    ...
    grade_result = await _hr_key_validator.run(
        question=current_question["question"],
    ...
    _log_cost(db, session_id, _hr_key_validator.name, grade_result.usage)
...
def get_result(db: Session, session_id: str) -> dict:
    ...
    if progress is None:
        raise HRServiceError("HR round not started for this session", status_code=409)
    if progress.status != "completed":
        raise HRServiceError("HR round not completed yet", status_code=409)
