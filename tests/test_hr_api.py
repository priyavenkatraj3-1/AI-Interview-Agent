"""
Integration tests for the HR round API (start / current / answer /
result), exercised through FastAPI's TestClient against a real (temp)
SQLite DB. The HR-question generator is replaced by the deterministic
`fake_hr_interviewer` fixture (autouse, see conftest.py) so these run
offline; grading goes through the real two-phase pipeline (see
agents/hr_interviewer/grader.py) whose phase-2 key validator is
MockHRKeyValidatorAgent (offline keyword heuristic, selected automatically
since MOCK_MODE defaults to true) — deterministic given the fake
generator's fixed rubric_keywords, so no further mocking is needed there,
mirroring the technical round's tests. See
tests/test_grader_independence.py for proof that phase 1 never sees the
answer key.
"""
from agents.hr_interviewer.taxonomy import MAX_DIFFICULTY, MIN_DIFFICULTY, START_DIFFICULTY, TOTAL_QUESTIONS
from app.services import hr_service

CORRECT_ANSWER = "I had a conflict, we talked to communicate, found a compromise, and managed to resolve it."
INCORRECT_ANSWER = "I have no idea."


def _answer(client, session_id, answer=CORRECT_ANSWER):
    return client.post(f"/api/sessions/{session_id}/hr/answer", json={"answer": answer})


def test_hr_generation_failure_returns_clean_502_not_a_crash(client, session_id, monkeypatch):
    async def always_fails(**kwargs):
        raise RuntimeError("simulated: mock fixture bank unavailable")

    monkeypatch.setattr(hr_service._hr_interviewer, "run", always_fails)

    response = client.post(f"/api/sessions/{session_id}/hr/start")
...
def test_unsupported_company_returns_400(client):
    ...
    sid = create.json()["id"]
    response = client.post(f"/api/sessions/{sid}/hr/start")
    assert response.status_code == 400


def test_double_start_is_idempotent(client, session_id, fake_hr_interviewer):
    first = client.post(f"/api/sessions/{session_id}/hr/start").json()
    second = client.post(f"/api/sessions/{session_id}/hr/start").json()
    assert first["question"]["question"] == second["question"]["question"]
    assert fake_hr_interviewer.calls == 1


def test_current_before_start_returns_409(client, session_id):
    response = client.get(f"/api/sessions/{session_id}/hr/current")
...
def test_result_before_completion_returns_409(client, session_id):
    ...
    response = client.get(f"/api/sessions/{session_id}/hr/result")
    assert response.status_code == 409


def test_session_persists_across_simulated_refresh(client, session_id):
    started = client.post(f"/api/sessions/{session_id}/hr/start").json()

    # Simulate a browser refresh: a fresh GET should return the SAME
    # pending question, not a newly generated one.
    resumed = client.get(f"/api/sessions/{session_id}/hr/current").json()
...
def test_full_round_is_exactly_total_questions_and_completes(client, session_id):
    ...
    result = client.get(f"/api/sessions/{session_id}/hr/result")
    ...
    # Stage transition: hr -> completed, the final stage, via the existing
    # orchestrator.
...
def test_history_entries_have_required_fields(client, session_id):
    ...
    history = client.get(f"/api/sessions/{session_id}/hr/result").json()["history"]
