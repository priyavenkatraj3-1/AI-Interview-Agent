"""
Unit tests for agents/hr_interviewer/hr_interviewer.py: the offline
MockHRInterviewerAgent (used when MOCK_MODE is enabled — the default,
since this project currently has no Anthropic API credits), the real
Claude-backed HRInterviewerAgent against a fake Anthropic client (no
network call), and the MOCK_MODE-driven factory switch.
"""
import pytest

from agents.hr_interviewer import hr_interviewer as hr_module
from agents.hr_interviewer.hr_interviewer import (
    HRInterviewerAgent,
    HRQuestionGenerationError,
    MockHRInterviewerAgent,
    build_hr_interviewer,
)
...
async def test_mock_generator_returns_a_valid_fixture_for_known_topic_pattern():
    agent = MockHRInterviewerAgent()
    result = await agent.run(
        target_company="TCS_NQT",
        topic="teamwork",
        pattern="team_conflict",
        difficulty=3,
        previous_questions=[],
    )
    ...
    assert result.data["topic"] == "teamwork"
    assert result.data["pattern"] == "team_conflict"
...
async def test_mock_generator_raises_for_unknown_topic_pattern():
    agent = MockHRInterviewerAgent()
    with pytest.raises(HRQuestionGenerationError):
        await agent.run(
            target_company="TCS_NQT",
            topic="unknown_topic",
...
async def test_mock_generator_fixtures_are_independent_copies():
    ...
    first = await agent.run(
        ...
        pattern="tell_me_about_yourself",
        difficulty=1,
        previous_questions=[],
    )
...
async def test_mock_generator_covers_every_taxonomy_topic_pattern():
    from agents.hr_interviewer.taxonomy import HR_TAXONOMY

    agent = MockHRInterviewerAgent()
    for topic, patterns in HR_TAXONOMY.items():
...
async def test_valid_structured_question_output():
    ...
    assert client.messages.calls[0]["tool_choice"] == {"type": "tool", "name": "emit_hr_question"}
...
async def test_duplicate_question_triggers_retry():
    ...
    fresh = {
        "question": "Tell me about a time you took initiative without being asked to.",
        "model_answer": "A strong answer describes the problem noticed, proactive action, and the impact.",
        "rubric_keywords": ["initiative", "proactive", "impact"],
    }
    ...
    result = await agent.run(
        target_company="TCS_NQT",
        topic="leadership_and_ownership",
        pattern="taking_initiative",
        difficulty=3,
...
def test_factory_returns_mock_agent_when_mock_mode_enabled(monkeypatch):
    monkeypatch.setattr(hr_module, "MOCK_MODE", True)
    agent = build_hr_interviewer()
    assert isinstance(agent, MockHRInterviewerAgent)


def test_factory_returns_real_agent_when_mock_mode_disabled(monkeypatch):
    monkeypatch.setattr(hr_module, "MOCK_MODE", False)
    agent = build_hr_interviewer()
    assert isinstance(agent, HRInterviewerAgent)
