"""Unit tests for agents/technical_interviewer/taxonomy.py — pure logic, no DB/API."""
import random
...
def test_topic_sequence_only_uses_known_topics():
    for company in SUPPORTED_COMPANIES:
        sequence = build_topic_sequence(company)
        assert set(sequence) <= set(TECHNICAL_TAXONOMY.keys())
...
def test_pick_pattern_avoids_used_patterns_when_possible():
    ...
    all_patterns = set(TECHNICAL_TAXONOMY[topic])
...
def test_pick_pattern_falls_back_to_reuse_once_exhausted():
    rng = random.Random(1)
    topic = "oop"
    used = set(TECHNICAL_TAXONOMY[topic])  # every pattern already used
    pattern = pick_pattern(topic, used, rng)
    assert pattern in TECHNICAL_TAXONOMY[topic]
