...
import { startTechnical, submitTechnicalAnswer } from './api/technical'

function TechnicalRound({ sessionId, onCompleted }) {
  ...
  // AptitudeRound/CodingRound's submittingRef.
  const submittingRef = useRef(false)

  useEffect(() => {
    let cancelled = false
    startTechnical(sessionId)
      .then((data) => {
        if (cancelled) return
        if (data.completed) {
          onCompleted()
  ...
  const handleSubmit = () => {
    ...
    setSubmitting(true)
    submitTechnicalAnswer(sessionId, answer)
  ...
  return (
    <div className="card">
      ...
      <label className="coding-editor-label" htmlFor="technical-answer">
        Your answer
      </label>
      <textarea
        id="technical-answer"
      ...
      {feedback && (
        <div className="feedback">
          <p>
            {feedback.is_correct ? '✅ Correct' : '❌ Needs improvement'} · Score: {feedback.answer_score}/100
          </p>
